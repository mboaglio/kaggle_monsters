...
setwd( "/Users/marcos/odrive/ITBA/Kaggle/monsters/XGBoostFE1")
...
dataset$color <- NULL  # Saco el color que jode
dataset$id <- NULL # Saco el id
# dataset$rotting_flesh # Lo dejo como esta
dataset$V1 <- (dataset$bone_length + dataset$hair_length + dataset$has_soul) / 3
...
library(corrplot)
attach(dataset)
boxplot(dataset$rotting_flesh ~ type, col = 'green', main= 'Rotting Flesh x type')

boxplot(dataset$hair_length ~ type, col = 'blue', main= 'Hair Length x type')
boxplot(dataset$bone_length ~ type, col = 'red', main= 'Bone length x type')
boxplot(dataset$has_soul ~ type, col = 'yellow', main= 'Has Soul x type')

# Armo una tabla de type vs color para ver que onda las variables 
type_color_dataset <- data.frame(dataset %>% group_by(type, color) %>% tally())
nn<-reshape(type_color_dataset,timevar="color",idvar="type",direction="wide")
rownames(nn) <- nn$type
nn$type <- NULL
View(nn)
heatmap(t(nn)[ncol(nn):1,], Rowv=NA, Colv=NA)
